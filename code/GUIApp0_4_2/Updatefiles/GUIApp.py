...
#     Generating and managing offline Bitcoin wallets.
...
class Menu(ctk.CTkFrame):	
	...
	def create_meny(self):
		...
		ExitButton = ctk.CTkButton(my_big_Frame, text="Close and exit", text_color="white", border_width=2, border_color="white", fg_color=System_button_color, width=120, height=32, font=my_font, command=self.encryptanddestroy)
	...
	def new_user_account_selection(self):
		...
		ExitButton = ctk.CTkButton(my_big_Frame, text="Close and exit", text_color="white", border_width=2, border_color="white", fg_color="navy blue", width=120, height=32, font=my_font, command=self.encryptanddestroy)
		ExitButton.place(relx=0.5, rely=0.95, anchor="center")
		...
		if path_to_USB_secure == 'Secure USB folder is not available':
			Button = ctk.CTkButton(my_Frame, text="Create a new account", text_color="white", fg_color="navy blue", border_width=2, border_color="white", height=36, font=my_font, command=self.new_secureUSB_Pre)
			Button.place(relx=0.5, rely=0.3, anchor="center")
			Button2 = ctk.CTkButton(my_Frame, text="Create a new account and a key for external communication", text_color="white", fg_color="navy blue", border_width=2, border_color="white", height=36, font=my_font, command=self.create_getStartedmeny)
			...
			Label2 = ctk.CTkLabel(my_Frame, text=PersonalGPGKey, text_color="white", fg_color="black", font=my_font)
			Label2.place(relx=0.45, rely=0.4, anchor="w")
	...
	def create_getStartedmeny(self):
		...
		Label1 = ctk.CTkLabel(my_Frame, text="This will let you set-up:", text_color="white", fg_color="black", font=my_font)
		...
		Label2 = ctk.CTkLabel(my_Frame, text="(1). An encrypted archive for important files, passwords and bitcoin wallets etc.", text_color="white", fg_color="black", font=my_font)
		Label2.place(relx=0.5, rely=0.24, anchor="center")
		Label3 = ctk.CTkLabel(my_Frame, text="(2). An encryption key for safe, and private, communication that can be used with emails etc.", text_color="white", fg_color="black", font=my_font)
		Label3.place(relx=0.5, rely=0.34, anchor="center")
		Label4 = ctk.CTkLabel(my_Frame, text="(3). A secondary backup on a USB-stick in case of breakage, fire or theft.", text_color="white", fg_color="black", font=my_font)
		Label4.place(relx=0.5, rely=0.44, anchor="center")
		my_font = ctk.CTkFont(family="Arial", size=20, weight="bold", slant="roman", underline=False, overstrike=False)
		Label5 = ctk.CTkLabel(my_Frame, text="(You will need a USB-stick ready to connect when instructed during the setup process.)", text_color="white", fg_color="black", font=my_font)
		Label5.place(relx=0.5, rely=0.52, anchor="center")
		my_font = ctk.CTkFont(family="Arial", size=22, weight="bold", slant="italic", underline=False, overstrike=False)
		Label6 = ctk.CTkLabel(my_Frame, text="(4). A Yubikey 5 NFC with subkeys for encrypting emails on computers and smartphones (optional).", text_color="white", fg_color="black", font=my_font)
		Label6.place(relx=0.5, rely=0.62, anchor="center")
		
		Button = ctk.CTkButton(my_Frame, text="Got it. Lets Go!", text_color="black", fg_color="pink", border_width=2, border_color="white", font=my_font, command=self.create_getStartedmeny_Pre)
	...
	def create_getStartedmeny2(self):	
		...
		def getStarted3():
			...
			full_path_statements = str(filepathdestinationfolder) + "/secure/wallets/statements/"
			...
			os.makedirs(os.path.dirname(full_path_statements))
		...
		def next_step4():
			...
			try:
				key = gpg.delete_keys(commkey, True, expect_passphrase=False) 
	...
	def getStartedfinnished(self):
		...
		Label = ctk.CTkLabel(my_Frame, text="Finished!", text_color="white", fg_color="black", font=my_font)
		Label.place(relx=0.5, rely=0.3, anchor="center")
		
		my_font = ctk.CTkFont(family="Arial", size=16, weight="bold", slant="italic", underline=True, overstrike=False)
		Label2 = ctk.CTkLabel(my_Frame, text="You have sucessfully created:", text_color="white", fg_color="black", font=my_font)
		Label2.place(relx=0.5, rely=0.37, anchor="center")
		...
		Button = ctk.CTkButton(my_Frame, text="Start using the offline device", text_color="black", fg_color="pink", border_width=2, border_color="white", font=my_font, command=self.create_meny)
	...
	def create_Boltcardmeny(self):
		...
		if path_to_USB_secure == 'Secure USB folder is available':
			...
			readButton = ctk.CTkButton(right_Frame, text="Scan QR", text_color="white", fg_color=Boltcard_button_color, border_width=2, border_color="white", width=150, height=20, font=my_font, command=self.readUUID_with_QR)
			...
			boltLabel = ctk.CTkLabel(right_Frame, text="Create a new Bolt card:", text_color="white", font=my_font, fg_color="black")
			...
			infoLabel13 = ctk.CTkLabel(right_Frame, text="4. Click \"New card\" and add Bolt card using the a file with above lines in it.", text_color="white", font=my_font, fg_color="black")
			...
			proLabel = ctk.CTkLabel(right_Frame, text="Use a unique key for receiving the signed Boltcard files from trusted LN Hub operator.", text_color="white", font=my_font, fg_color="black")
			proLabel.place(relx=0.06, rely=0.85, anchor="w")
			proLabel2 = ctk.CTkLabel(right_Frame, text="Use button \"Scan QR\" above to input UID to create a file that's encrypted and signed.", text_color="white", font=my_font, fg_color="black")
				...
				with open(completeName, 'r') as file:
					...
					for lines in csvfile:
						if lines:
							my_font = ctk.CTkFont(family="Arial", size=18, slant="roman", underline=False, overstrike=False)
							if Counting == 0:	
								Button1 = ctk.CTkButton(left_Frame, text="", image = IDimagebutton, anchor='center', text_color="white", fg_color="brown", border_width=2, border_color="brown", width=220, height=100, font=my_font, command=partial(self.showcard, lines[2]))
								...
								Button2 = ctk.CTkButton(left_Frame, text="", image = IDimagebutton, anchor='center', text_color="white", fg_color="brown", border_width=2, border_color="brown", width=220, height=100, font=my_font, command=partial(self.showcard, lines[2]))
								Button2.pack(padx=10, pady=8, side= TOP, anchor="w")
	...
	def showcard(self, cardidentification):
		...
		def exportCard():
			...
			if os.path.isfile(pathtofile):
				...
				time.sleep(2)
				outputdir = filedialog.askdirectory(initialdir='/media/user1')
				pathtofileOut = outputdir + '/Exported_Boltcard.txt'
				shutil.copy(pathtofile, pathtofileOut)
				tk.messagebox.showinfo('Information', 'Boltcard file has been saved.')
			else:
				tk.messagebox.showinfo('Alert!', 'This Boltcard has no pre-defined data file.')
		...
		if path_to_USB_secure == 'Secure USB folder is available':
					...
					for lines in csvfile:
						if lines:
							if lines[2] == cardidentification:
								...
								found = True	
							my_font = ctk.CTkFont(family="Arial", size=14, slant="roman", underline=False, overstrike=False)
							Button1 = ctk.CTkButton(left_Frame, text="", image = boltcardiconimage, anchor='center', text_color="white", fg_color="brown", border_width=2, border_color="white", width=220, height=100, font=my_font, command=partial(self.showcard, lines[2]))
							Button1.pack(padx=10, pady=8, side= TOP, anchor="w")
							Llabel1 = ctk.CTkLabel(left_Frame, text=lines[0], text_color="white", font=my_font, fg_color="black")
							Llabel1.pack(padx=1, pady=0, side= TOP, anchor="center")
				file.close()
			...
			if found:
				...
				programButton2 = ctk.CTkButton(right_Frame, text="Program (Smartphone)", text_color="white", fg_color="brown", border_width=2, border_color="white", width=240, height=32, font=my_font, command=partial(self.programcard, cardidentification))
	...
	def createPassword(self, combinedValueBase58):
		...
		doButton = ctk.CTkButton(my_Frame, text="CLICK HERE! To Start creating password!", text_color="white", fg_color="brown", border_width=2, border_color="white", width=240, height=32, font=my_font, command=partial(self.dicePassword, "1"))
		...
		Label22.place(relx=0.4, rely=0.33, anchor="e")
		...
		Real_name.place(relx=0.41, rely=0.33, anchor="w")
		
		Label6 = ctk.CTkLabel(my_Frame, text="View/edit password:", font=my_font, text_color="white", fg_color="black")
		Label6.place(relx=0.4, rely=0.39, anchor="e")
		if combinedValueBase58 != "none":
			strong_password_var = ctk.StringVar(value=combinedValueBase58)
			...
			Secret_passphrase.place(relx=0.41, rely=0.39, anchor="w")
			copy_button = ctk.CTkButton(my_Frame, text="Copy to clipboard", text_color="white", fg_color="dark green", height=25, width=150, border_width=1, border_color="white", font=my_font, command=partial(self.copy2clip, combinedValueBase58))
			copy_button.place(relx=0.67, rely=0.46, anchor="center")
		else:
			...
			Secret_passphrase.place(relx=0.41, rely=0.39, anchor="w")
		tButton = ctk.CTkButton(my_Frame, text="Show/hide", font=my_font, text_color="black", fg_color="pink", border_width=2, border_color="white", command=tPassphrase)
		tButton.place(relx=0.82, rely=0.39, anchor="center")
		
		Label4 = ctk.CTkLabel(my_Frame, text="Comment:", font=my_font, text_color="white", fg_color="black")
		Label4.place(relx=0.4, rely=0.53, anchor="e")
		...
		comment_entry.place(relx=0.41, rely=0.53, anchor="w")
		
		doButton = ctk.CTkButton(my_Frame, text="Save the password to the Secure archive", text_color="white", fg_color="brown", border_width=2, border_color="white", width=240, height=32, font=my_font, command=do_newPassword)
	...
	def readUUID_with_QR(self):
		...
		def do_save():
			...
			textfilename = 'Boltcard_UID.txt'
			
			c = open(outputdir + '/' + textfilename, 'w')
			c.write(msg)
			c.close()
			messagebox.showinfo('Information', 'File \"' + textfilename + '\" has been saved.')
			self.create_Boltcardmeny()

		
		def do_readUUID_with_QR():
			global bolt_uid
			backg = ctk.CTkImage(light_image=Image.open(pathtobackg), dark_image=Image.open(pathtobackg), size=(1200, 650))
			Label_backg = ctk.CTkLabel(my_Frame, image=backg, text = "")
			Label_backg.place(x=0, y=0)
			...
			subprocess.run(['python scanQRmessage.py'], shell=True)
			...
			my_font = ctk.CTkFont(family="Arial", size=24, slant="roman", underline=True, overstrike=False)
			uuidLabel = ctk.CTkLabel(my_Frame, text='UID from card:', text_color="white", font=my_font, fg_color="black")
			...
			uuidLabel2 = ctk.CTkLabel(my_Frame, text=msg, text_color="white", font=my_font, fg_color="black")
			uuidLabel2.place(relx=0.5, rely=0.28, anchor="center")
			
			# Generate QR-codes for Public address and display on screen
			load_address = qrcode.make(msg, version=1)
			
			resize_load_address = load_address.resize((250, 250))
			pathtoloadaddress = str(filepathdestinationfolder) + "/secure/boltcards/load.png"
			...
			pubLabel = ctk.CTkLabel(my_Frame, text="UID number:", text_color="white", font=my_font, fg_color="black")
			pubLabel.place(relx=0.34, rely=0.38, anchor="center")
			Labelpublicimg = ctk.CTkLabel(my_Frame,  text = "", image = loadimg)
			Labelpublicimg.place(relx=0.34, rely=0.6, anchor="center")
				
			my_font = ctk.CTkFont(family="Arial", size=20, slant="roman", underline=False, overstrike=False)
			saveButton = ctk.CTkButton(my_Frame, text="Save as plain text", text_color="white", fg_color="brown", border_width=2, border_color="white", width=270, height=32, font=my_font, command=do_save)
			saveButton.place(relx=0.66, rely=0.54, anchor="center")
			encryptButton = ctk.CTkButton(my_Frame, text="Encrypt and sign", text_color="white", fg_color="brown", border_width=2, border_color="white", width=270, height=32, font=my_font, command=partial(self.encrypt_and_sign_UID, msg))
			encryptButton.place(relx=0.66, rely=0.62, anchor="center")
		...
		Label = ctk.CTkLabel(my_Frame, text="Input a Boltcard's UID (Unique User Identification number) using a QR-code", text_color="white", font=my_font, fg_color="black")
		Label.place(relx=0.5, rely=0.2, anchor="center")
		my_font = ctk.CTkFont(family="Arial", size=20, slant="roman", underline=False, overstrike=False)

		my_font = ctk.CTkFont(family="Arial", size=22, slant="roman", underline=False, overstrike=False)
		Label3 = ctk.CTkLabel(my_Frame, text='After clicking the \"Start\"- button, hold the QR-code (with UID) infront of the camera.', text_color="white", font=my_font, fg_color="black")
		Label3.place(relx=0.5, rely=0.34, anchor="center")
		...
		my_font = ctk.CTkFont(family="Arial", size=20, slant="roman", underline=False, overstrike=False)
		startButton = ctk.CTkButton(my_Frame, text="Start", text_color="white", fg_color="brown", border_width=2, border_color="white", width=170, height=32, font=my_font, command=do_readUUID_with_QR)
	...
	def encrypt_and_sign_UID(self, UID):
		...
		def do_encrypt_and_sign_UID():
			...
			# Encrypt the file
			gpg = gnupg.GPG(gnupghome='/home/user1/.gnupg/')
			encrypted_data = gpg.encrypt(UID, clicked_publicKey, sign=clicked_privateKey) 
	...
	def programcard(self, cardidentification):
		global filepathdestinationfolder
		completeName = str(filepathdestinationfolder) + "/secure/boltcards/boltcards.csv"
		updatedcompleteName = str(filepathdestinationfolder) + "/secure/boltcards/updatedboltcards.csv"
				...
				for lines in csvfile:
					if lines:
						if lines[2] == cardidentification:
							...
							pubLabel = ctk.CTkLabel(my_Frame, text="Program card using smartphone app *", text_color="white", font=my_font, fg_color="black")
							pubLabel.place(relx=0.5, rely=0.07, anchor="center")
							my_font = ctk.CTkFont(family="Arial", size=22, slant="roman", underline=False, overstrike=False)
							pubLabel2 = ctk.CTkLabel(my_Frame, text="(* Bolt Card NFC Card Creator- app from App/Play-store)", text_color="white", font=my_font, fg_color="black")
							pubLabel2.place(relx=0.5, rely=0.12, anchor="center")
	...
	def create_BitcoinWalletmeny(self):
		...
		if path_to_USB_secure == 'Secure USB folder is available':
			...
			Buttoncreate = ctk.CTkButton(right_Frame, text="Create new Wallet", text_color="black", fg_color="orange", border_width=2, border_color="white", width=150, height=20, font=my_font, command=self.createnewBitcoinwallet_PRE)
			Buttoncreate.place(relx=0.9, rely=0.10, anchor="center")
			pathtodiceicon = str(filepathdestinationfolder) + "/images/dice_icon.jpg"
			diceIconImage = ctk.CTkImage(light_image=Image.open(pathtodiceicon), dark_image=Image.open(pathtodiceicon), size=(75, 90))
			dicepicButton = ctk.CTkButton(right_Frame, text="", image = diceIconImage, anchor='center', text_color="white", fg_color="black", border_width=0, border_color="black", width=75, height=90, font=my_font, command=self.diceWallet_PRE)
			dicepicButton.place(relx=0.94, rely=0.2, anchor='center')
			...
			my_font = ctk.CTkFont(family="Arial", size=20, slant="roman", underline=True, overstrike=False)
			infoLabel = ctk.CTkLabel(right_Frame, text="Offline device has two options for Bitcoin wallets with air-gapped signing feature:", text_color="white", font=my_font, fg_color="black")
			infoLabel.place(relx=0.03, rely=0.15, anchor="w")
			
			my_font = ctk.CTkFont(family="Arial", size=20, slant="roman", underline=False, overstrike=False)
			infoLabel = ctk.CTkLabel(right_Frame, text="1. Add a wallet that's created by a seperate hardware wallet *:", text_color="white", font=my_font, fg_color="black")
			infoLabel.place(relx=0.03, rely=0.2, anchor="w")
			infoLabel2 = ctk.CTkLabel(right_Frame, text="  - You could use a Blockstream Jade that can generate wallets offline.", text_color="white", font=my_font, fg_color="black")
			infoLabel2.place(relx=0.03, rely=0.26, anchor="w")
			infoLabel3 = ctk.CTkLabel(right_Frame, text="  - The Offline device will hold the seed so it can be backed-up etc.", text_color="white", font=my_font, fg_color="black")
			infoLabel3.place(relx=0.03, rely=0.32, anchor="w")
			infoLabel4 = ctk.CTkLabel(right_Frame, text="  - The seed is encrypted when powering down or logging out.", text_color="white", font=my_font, fg_color="black")
			infoLabel4.place(relx=0.03, rely=0.38, anchor="w")
			
			infoLabel10 = ctk.CTkLabel(right_Frame, text="2. Create a new wallet on the Offline device *:", text_color="white", font=my_font, fg_color="black")
			infoLabel10.place(relx=0.03, rely=0.46, anchor="w")
			infoLabel11 = ctk.CTkLabel(right_Frame, text="  - This could have less randomness and be less secure!", text_color="white", font=my_font, fg_color="black")
			infoLabel11.place(relx=0.03, rely=0.52, anchor="w")
			infoLabel12 = ctk.CTkLabel(right_Frame, text="  - The Offline device can generate as many wallets as you like.", text_color="white", font=my_font, fg_color="black")
			infoLabel12.place(relx=0.03, rely=0.58, anchor="w")
			infoLabel13 = ctk.CTkLabel(right_Frame, text="  - The seed is encrypted when powering down or logging out.", text_color="white", font=my_font, fg_color="black")
			infoLabel13.place(relx=0.03, rely=0.64, anchor="w")
			infoLabel14 = ctk.CTkLabel(right_Frame, text="Wallet Xpub are moved to an Online device for creating and broacasting the transactions **.", text_color="white", font=my_font, fg_color="black")
			infoLabel14.place(relx=0.03, rely=0.72, anchor="w")
			...
			proLabel3 = ctk.CTkLabel(right_Frame, text="** Only the transaction signing is done offline (using Blockstream Jade of Offline device etc)", text_color="white", font=my_font, fg_color="black")
	...
	def diceWallet(self, diceroll):
		...
		if int(diceroll) <= 23:
			...
			calcButton = ctk.CTkButton(my_Frame, text="Create wallet made from dice throws", text_color="white", fg_color="dark green", border_width=2, border_color="white", width=170, height=32, font=my_font, command=createWallet)
			calcButton.place(relx=0.5, rely=0.86, anchor="center")
	...
	def dicePassword(self, diceroll):
		...
		drop1 = OptionMenu(my_Frame, opt1, *linesList)
		drop1.config(width=3)
		drop1.place(relx=0.12, rely=0.3, anchor="center")
		
		drop2 = OptionMenu(my_Frame, opt2, *linesList)
		drop2.config(width=3)
		drop2.place(relx=0.19, rely=0.3, anchor="center")
		
		drop3 = OptionMenu(my_Frame, opt3, *linesList)
		drop3.config(width=3)
		drop3.place(relx=0.26, rely=0.3, anchor="center")
		
		drop4 = OptionMenu(my_Frame, opt4, *linesList)
		drop4.config(width=3)
		drop4.place(relx=0.33, rely=0.3, anchor="center")
		
		drop5 = OptionMenu(my_Frame, opt5, *linesList)
		drop5.config(width=3)
		drop5.place(relx=0.42, rely=0.3, anchor="center")
		
		drop6 = OptionMenu(my_Frame, opt6, *linesList)
		drop6.config(width=3)
		drop6.place(relx=0.49, rely=0.3, anchor="center")
		
		drop7 = OptionMenu(my_Frame, opt7, *linesList)
		drop7.config(width=3)
		drop7.place(relx=0.56, rely=0.3, anchor="center")
		
		drop8 = OptionMenu(my_Frame, opt8, *linesList)
		drop8.config(width=3)
		drop8.place(relx=0.63, rely=0.3, anchor="center")
		
		drop9 = OptionMenu(my_Frame, opt9, *linesList)
		drop9.config(width=3)
		drop9.place(relx=0.12, rely=0.37, anchor="center")
		
		drop10 = OptionMenu(my_Frame, opt10, *linesList)
		drop10.config(width=3)
		drop10.place(relx=0.19, rely=0.37, anchor="center")
		
		drop11 = OptionMenu(my_Frame, opt11, *linesList)
		drop11.config(width=3)
		drop11.place(relx=0.26, rely=0.37, anchor="center")
		
		drop12 = OptionMenu(my_Frame, opt12, *linesList)
		drop12.config(width=3)
		drop12.place(relx=0.33, rely=0.37, anchor="center")
		
		drop13 = OptionMenu(my_Frame, opt13, *linesList)
		drop13.config(width=3)
		drop13.place(relx=0.42, rely=0.37, anchor="center")
		
		drop14 = OptionMenu(my_Frame, opt14, *linesList)
		drop14.config(width=3)
		drop14.place(relx=0.49, rely=0.37, anchor="center")
		
		drop15 = OptionMenu(my_Frame, opt15, *linesList)
		drop15.config(width=3)
		drop15.place(relx=0.56, rely=0.37, anchor="center")
		
		drop16 = OptionMenu(my_Frame, opt16, *linesList)
		drop16.config(width=3)
		drop16.place(relx=0.63, rely=0.37, anchor="center")
		
		nextdiceroll = int(diceroll) + 1
		nextButton = ctk.CTkButton(my_Frame, text="Add set " + str(diceroll), text_color="white", fg_color="black", border_width=2, border_color="white", width=170, height=32, font=my_font, command=addEntropy)
		nextButton.place(relx=0.86, rely=0.34, anchor="center")
					...
					for lines in csvfile:
						if lines[0] == walletname:	
							...
							if transactiondatoavailable and file_content != "Empty":
								...
								signQRButton = ctk.CTkButton(right_Frame, text="SIGN", text_color="white", fg_color="orange", border_width=2, border_color="white", width=200, height=34, font=my_font, command=do_sign_) 
		...
		try:
			with open(completeName, 'r') as file:
				...
				for lines in csvfile:
					if lines[0] == walletname:	
						my_font = ctk.CTkFont(family="Arial", size=26, weight="bold", slant="roman", underline=False, overstrike=False)
						nameLabel = ctk.CTkLabel(my_Frame, text=lines[0] + ' - Statement', text_color="white", font=my_font, fg_color="black")
						...
						if os.path.isfile(pathtoicon):
							iconImage = ctk.CTkImage(light_image=Image.open(pathtoicon), dark_image=Image.open(pathtoicon), size=(100, 100))
							Buttonicon = ctk.CTkButton(my_Frame, text='', image = iconImage, compound=TOP, anchor='center', text_color="white", fg_color="black", corner_radius=0, border_width=0, border_color="black", width=100, height=100, font=my_font, command=partial(self.editWalletInformation, walletname))
							Buttonicon.place(relx=0.9, rely=0.12, anchor="center")
						...
						backButton = ctk.CTkButton(my_Frame, text="Save", text_color="black", fg_color="orange", border_width=2, border_color="white", width=170, height=32, font=my_font, command=save_statement)
	...
	def scanForAddress(self, walletname):
		...
		subprocess.run(['python scanQRforAddress.py'], shell=True)
	...
	def addLineToStatement(self, walletname):
		...
		def do_addLineToStatement():
			...
			try:
				...
				tk.messagebox.showinfo('Information', 'The inputed amount needs to be a (even) number".')
				self.addLineToStatement(walletname)
			...
			try:
				...
				tk.messagebox.showinfo('Information', 'The inputed fee needs to be a (even) number".')
				self.addLineToStatement(walletname)
		...
		signQRButton.place(relx=0.53, rely=0.42, anchor="e")
		...
		my_font = ctk.CTkFont(family="Arial Black", size=22, weight="bold", slant="roman", underline=False, overstrike=False)
		addButton = ctk.CTkButton(my_Frame, text="Add", text_color="black", fg_color="orange", border_width=2, border_color="white", width=140, height=34, font=my_font, command=do_addLineToStatement) #command=partial(self.decodeQRfromCamera_step2, walletname))
		addButton.place(relx=0.92, rely=0.52, anchor="center")
		my_font = ctk.CTkFont(family="Arial Black", size=14, weight="bold", slant="roman", underline=False, overstrike=False)
		infoLabel = ctk.CTkLabel(my_Frame, text="* For accounting purposes etc.", text_color="white", fg_color="black", font=my_font)
					...
					if lines:
						...
						for i in private_keys:
							if lines[1] == i['fingerprint']: 
								...
								pathtoIDimage = str(filepathdestinationfolder) + "/ID/picture" + lines[1] + ".jpg"
								if os.path.isfile(pathtoIDimage):
									...
									pathtoIDimage = str(filepathdestinationfolder) + "/Pictures/male_anon_ID.JPG"
								if Counting == 0:
									FirstID = lines[1]
									Counting += 1
		except FileNotFoundError:
	...
	def showID(self, IDfingerprint):
		...
		def export_image():
			...
			full_path_back = outputdir + '/ID_' + IDfingerprint + '_backside.png'
			pathtoIDbackside = "/home/user1/images/ID_map_back.png"
			...
			if os.path.isfile(pathtoIDbackside):
				shutil.copy(pathtoIDbackside, full_path_back)
		...
		def flipID():
			pathtobackg = "/home/user1/images/ID_map_back.png"
			backg = ctk.CTkImage(light_image=Image.open(pathtobackg), dark_image=Image.open(pathtobackg), size=(720, 460))
			Label_backg = ctk.CTkLabel(ID_Frame, image=backg, text = "")
			Label_backg.place(x=0, y=0)
			ID_Frame.lift()
			my_font = ctk.CTkFont(family="Arial", size=20, weight="bold", slant="roman", underline=False, overstrike=False)
			flipToFrontButton = ctk.CTkButton(right_Frame, text="Flip", text_color="white", fg_color="brown", border_width=2, border_color="white", width=80, height=22, font=my_font, command=partial(self.showID, IDfingerprint))
			flipToFrontButton.place(relx=0.84, rely=0.8, anchor="center")
				...
				with open(completeName, 'r') as file:
					...
					for lines in csvfile:
						if lines[1] == IDfingerprint:
							...
							exportButton.place(relx=0.12, rely=0.03, anchor="center")
							...
							if newIDflag:
								...
								valiLabel = ctk.CTkLabel(right_Frame, text="ID-key validations (signatures): ", text_color="black", font=my_font, fg_color="old lace")
								valiLabel.place(x=80, y=510, anchor="nw")
	...
	def showID_more(self, IDfingerprint):
		...
		bottom_Frame._scrollbar.configure(height=0)
		bottom_Frame.place(x=40, y=485, anchor="nw")
		...
		if path_to_USB_secure == 'Secure USB folder is available':
			...
			my_font = ctk.CTkFont(family="Arial", size=30, weight="bold", slant="roman", underline=True, overstrike=False)
			infoLabel = ctk.CTkLabel(my_Frame, text="Standard ID: ", text_color="black", font=my_font, fg_color="old lace")
			infoLabel.place(relx=0.5, rely=0.06, anchor="center")
			my_font = ctk.CTkFont(family="Arial", size=26, weight="bold", slant="italic", underline=False, overstrike=False)
			info2Label = ctk.CTkLabel(my_Frame, text=name_ID, text_color="black", font=my_font, fg_color="old lace")
			...
			addValidationButton.place(relx=0.54, rely=0.24, anchor="center")
			...
			addSubkeyButton.place(relx=0.54, rely=0.34, anchor="center")
			...
			photoButton.place(relx=0.54, rely=0.44, anchor="center")
			detailsButton = ctk.CTkButton(my_Frame, text="View key details", text_color="white", fg_color="brown", border_width=2, border_color="white", width=250, height=36, font=my_font, command=partial(self.get_IDKey_details, IDfingerprint))
			detailsButton.place(relx=0.54, rely=0.54, anchor="center")
			
			validateButton = ctk.CTkButton(my_Frame, text="Validate someone´s ID", text_color="white", fg_color="brown", border_width=2, border_color="white", width=250, height=36, font=my_font, command=partial(self.sign_ID_key_on_keychain, IDfingerprint))
			validateButton.place(relx=0.8, rely=0.24, anchor="center")
			...
			signFileButton.place(relx=0.8, rely=0.34, anchor="center")
			...
			signQRButton.place(relx=0.8, rely=0.44, anchor="center")
			...
			exportIDButton.place(relx=0.8, rely=0.54, anchor="center")
			
			deleteButton = ctk.CTkButton(my_Frame, text="Delete ID", text_color="white", fg_color="red4", border_width=2, border_color="white", width=200, height=36, font=my_font, command=partial(self.do_deleteID, IDfingerprint))
			deleteButton.place(relx=0.67, rely=0.66, anchor="center")
			
			my_font = ctk.CTkFont(family="Arial", size=18, weight="bold", slant="roman", underline=False, overstrike=False)
			
			validatingLabel = ctk.CTkLabel(my_Frame, text="ID-key validations (signatures): ", text_color="black", font=my_font, fg_color="old lace")
			validatingLabel.place(x=42, y=454, anchor="nw")
			...
			backButton.place(relx=0.92, rely=0.04, anchor="center")
		else:
			notOKButton = ctk.CTkButton(self, text="You are not logged in. Log in now?", text_color="white", fg_color="purple", border_width=2, border_color="white", width=170, height=32, font=my_font, command=self.decrypt_SecUSB)
			notOKButton.place(relx=0.5, rely=0.5, anchor="center")
	...
	def changePhotoID(self, IDfingerprint):
		...
		tk.messagebox.showinfo('Information', 'Connect the USB-device and then click \"OK\".')
		time.sleep(2)
		messagebox.showinfo("Information", "Select your picture (\".jpg\" or \".png\" format) for the ID.")
		time.sleep(2)
	...
	def get_IDKey_details(self, IDfingerprint):		
		...
		Label1 = ctk.CTkLabel(my_Frame, text="Digital ID's key details", text_color="white", fg_color="black", font=my_font)
		Label1.pack(padx=35, pady=2, side= TOP, anchor="w")
		my_font = ctk.CTkFont(family="Arial", size=20, slant="roman", underline=False, overstrike=False)
		
		back_Button = ctk.CTkButton(my_Frame, text="Back", text_color="white", fg_color="brown", border_width=2, border_color="white", height=30, font=my_font, command=partial(self.showID_more, IDfingerprint))
		back_Button.place(x=970, y=40, anchor="nw")
		
		my_font = ctk.CTkFont(family="Arial", size=22, slant="roman", underline=False, overstrike=False)
		
		Label2 = ctk.CTkLabel(my_Frame, text="(Encryption standard: RFC 4880. Key size: 4096 bits.)", text_color="white", fg_color="black", font=my_font)
		Label2.pack(padx=35, pady=2, side= TOP, anchor="w")
		
		Label3 = ctk.CTkLabel(my_Frame, text="The private key for the digital ID is placed on the local keychain. Keep it offline!", text_color="white", fg_color="black", font=my_font)
		Label3.pack(padx=35, pady=5, side= TOP, anchor="w")
		...
		for i in private_keys:
			if IDfingerprint == i['fingerprint'] and self.lookup_Alias_absolut(i['fingerprint']) != "None":			
				my_font = ctk.CTkFont(family="Arial", size=14, slant="roman", underline=False, overstrike=False)
				Label4 = ctk.CTkLabel(my_Frame, text=str(i['uids']) + ' ' + str(i['fingerprint']) + '  (Alias=' + self.lookup_Alias(i['fingerprint']) + ')' + '\n', text_color="white", fg_color="black", font=my_font)
				Label4.pack(padx=25, pady=2, side= TOP, anchor="w")
				my_font = ctk.CTkFont(family="Arial", size=16, weight="bold", slant="roman", underline=False, overstrike=False)
			elif IDfingerprint == i['fingerprint']:
				my_font = ctk.CTkFont(family="Arial", size=14, slant="roman", underline=False, overstrike=False)
				Label5 = ctk.CTkLabel(my_Frame, text=str(i['uids']) + ' ' + str(i['fingerprint']) + '\n', text_color="white", fg_color="black", font=my_font)
				Label5.pack(padx=25, pady=2, side= TOP, anchor="w")
				my_font = ctk.CTkFont(family="Arial", size=16, weight="bold", slant="roman", underline=False, overstrike=False)
			...
			if i['subkeys']: 
				...
				for ii in listoflistindict:
					if ii:
						...
						if token != '#': 
							if token == '>': 
								Label6 = ctk.CTkLabel(my_Frame, text= '         ' + '\"' + capacity + '\"' + ' subkey: ' + ii[2] + '. Created: ' + dt_datecrea_form + '. Expires: ' + dt_dateexp_form + ' (Moved to a Yubikey).', text_color="white", fg_color="black", font=my_font)
								Label6.pack(padx=25, pady=2, side= TOP, anchor="w")
							else:
								Label7 = ctk.CTkLabel(my_Frame, text= '         ' + '\"' + capacity + '\"' + ' subkey: ' + ii[2] + '. Created: ' + dt_datecrea_form + '. Expires: ' + dt_dateexp_form + ' (' + keytype + ').', text_color="white", fg_color="black", font=my_font)
								Label7.pack(padx=25, pady=2, side= TOP, anchor="w")
	...
	def signQRfromCamera_ID(self, fingerprint):
		...
		if path_to_USB_secure == 'Secure USB folder is available':			
			...
			if os.path.isfile(link_to_signature_data):
				...
				saveQRButton.place(relx=0.5, rely=0.8, anchor="center")
				backButton = ctk.CTkButton(my_Frame, text="Back", text_color="white", fg_color="brown", border_width=2, border_color="white", width=170, height=32, font=my_font, command=partial(self.showID_more, fingerprint))
				backButton.place(relx=0.5, rely=0.96, anchor="center")
			else:
				my_font = ctk.CTkFont(family="Arial", size=28, slant="roman", underline=False, overstrike=False)
				pubLabel = ctk.CTkLabel(my_Frame, text="Could not read QR-data", text_color="white", font=my_font, fg_color="black")
	...
	def sign_ID_key_on_keychain(self, IDfingerprint):
		...
		Label1 = ctk.CTkLabel(my_Frame, text="Signing an ID public key using your own ID's master key.", text_color="white", fg_color="black", font=my_font)
		Label1.place(relx=0.5, rely=0.22, anchor="center")
		my_font = ctk.CTkFont(family="Arial", size=20, weight="bold", slant="roman", underline=False, overstrike=False)
		
		Label2 = ctk.CTkLabel(my_Frame, text="(1). Select the public key that you would like to sign.", text_color="white", fg_color="black", font=my_font)
		Label2.place(relx=0.5, rely=0.30, anchor="center")
		
		# Check if the ID master key has a signing subkey???
		
		answer = messagebox.askquestion('Important!', 'Is the public key you like to sign on a USB-device (not yet imported)?')
		if answer == 'yes':
			tk.messagebox.showinfo('Information', 'Connect the USB-device and then click \"OK\".')
			time.sleep(2)
		...
		Label5 = ctk.CTkLabel(my_Frame, text="The private ID key to sign with:", text_color="white", fg_color="black", font=my_font)
		...
		Label52.place(relx=0.41, rely=0.5, anchor="w")
	...
	def create_newID(self):
		...
		my_font = ctk.CTkFont(family="Arial", size=20, weight="bold", slant="roman", underline=False, overstrike=False)
		infoLabel = ctk.CTkLabel(my_Frame, text="A Digital ID is based on an existing key on your local keychain.", text_color="white", fg_color="black", font=my_font)
		infoLabel.place(relx=0.48, rely=0.16, anchor="center")
		infoLabel2 = ctk.CTkLabel(my_Frame, text="Don't use any of your regular keys that is for external communication for your Digital ID!", text_color="white", fg_color="black", font=my_font)
		infoLabel2.place(relx=0.48, rely=0.21, anchor="center")
		
		nameLabel = ctk.CTkLabel(my_Frame, text="First name:", text_color="white", fg_color="black", font=my_font)
		nameLabel.place(relx=0.34, rely=0.29, anchor="e")
		...
		address2Entry.place(relx=0.72, rely=0.51, anchor="e")
		...
		Button.place(relx=0.61, rely=0.86, anchor="w")
	...
	def create_getSettimetextbox(self):
			...
			if checkbox.get() == "on":					
				...
				if not os.path.isfile(path_to_externalAliases):
					if clicked_Avatar == 'Upload my own':
						...
						time.sleep(2)
						messagebox.showinfo("Information", "Select the picture for your avatar (\".jpg\" or \".png\" format)")
						time.sleep(2)
						pathtopicture = filedialog.askopenfilename(initialdir='/media/user1')
					...
					if clicked_Avatar == 'Upload my own':
						tk.messagebox.showinfo('Information', 'Connect the USB-device and then click \"OK\".')
						time.sleep(2)
						messagebox.showinfo("Information", "Select the picture for your avatar (\".png\" or \".jpg\" format)")
						time.sleep(2)
						pathtopicture = filedialog.askopenfilename(initialdir='/media/user1')
						newpicturename = PersonalGPGKey + '.png'
						pathtopicturelocation = filepathdestinationfolder + "/Documents/" + PersonalGPGKey + '.png'
		...
		if path_to_USB_secure == 'Secure USB folder is available':	
			...
			updateButton.place(relx=0.32, rely=0.74, anchor="w")
	...
	def create_Hometextbox(self):
		...
		if path_to_USB_secure == 'Secure USB folder is available':
			...
			if os.path.isfile(pathInheritanceMessage):
				...
				welcomelabel = ctk.CTkLabel(self, text="In memory of  " + logged_in_user, text_color="white", fg_color=main_frame_bg_mid, font=my_font)
			...
			if os.path.isfile(pathInheritanceMessage):
				...
				Labelinfo.place(relx=0.15, rely=0.98, anchor="w")
			...
			if not List_fingerprints:	
				startButton = ctk.CTkButton(my_Frame, text=" Start ", corner_radius=25, text_color="white", fg_color=System_button_color, border_width=3, border_color="white", font=my_font, command=self.new_secureUSB_Pre) #  new_user_account_selection
				...
				LoginButton = ctk.CTkButton(my_Frame, text=" Create new account", corner_radius=15, text_color="white", fg_color=System_button_color, border_width=3, border_color="white", font=("Arial", 18), command=self.new_secureUSB_Pre) #  new_user_account_selection
	...
	def get_GnuPGKeys_compact(self):		
		...
		my_font = ctk.CTkFont(family="Arial", size=18, weight="bold", slant="roman", underline=False, overstrike=False)
		# Info explained
		infoLabel = ctk.CTkLabel(my_Frame,text="* To add/remove aliases etc.", text_color="white", fg_color="black", font=my_font)
		infoLabel.place(x=940, y=80, anchor="nw")
	...
	def newGPGFull_Key(self, combinedValueBase58):
		...
		def limitSizePassphrase_text(*args):
			value = Secret_passphrase.get()
			if len(value) > 42: Secret_passphrase.set(value[:42])
		...
		Name_email = ctk.CTkEntry(my_Frame, placeholder_text="bob.smaith@cyb.org", font=my_font, width=320, height=25, text_color="white", border_color="white", border_width=3, corner_radius=8)
		...
		Button.place(relx=0.46, rely=0.51, anchor="w")
		if privatekeysavailable == True:
			...
			Button2.place(relx=0.56, rely=0.63, anchor="w")
		...
		if publickeysavailable == True:
			...
			Button2.place(relx=0.57, rely=0.81, anchor="w")
	...
	def decrypt_from_SecUSB(self):
		...
		if path_to_USB_secure == 'Secure USB folder is available':
			...
			ButtonRestore= ctk.CTkButton(my_Frame, text="Start", text_color="white", fg_color=SecUSB_button_color, border_width=2, border_color="white", width=170, height=32, font=my_font, command=self.decrypt_message)
			...
			try:
				...
				if data_.ok:
					...
					if not List_fingerprints:	
						...
						Button44.place(relx=0.39, rely=0.94, anchor="w")
	...
	def importGPG_Key(self):
		...
		Label3 = ctk.CTkLabel(my_Frame, text="(Don't forget to make a backup of the local keychain to the secure archive after importing.)", text_color="white", fg_color="black", font=my_font)
		...
		import_result = gpg.import_keys_file(filepathSourcefile)
		tk.messagebox.showinfo('Information', 'Tips: A new key can be given an Alias under \"GPG -> Check local keys\".\n\n Also, don\'t forget to make a backup of any new key to the secure archive.')
	...
	def do_all_keys(self):
		...
		if listofimportedfingerprints:
			infoLabel = ctk.CTkLabel(my_S_Frame, text="The following keys were imported from Secure archive to the local keychain:", text_color="light green", fg_color="black", font=my_font)
			infoLabel.pack(padx=5, pady=1, side= TOP, anchor="w")
			for i2 in listofimportedfingerprints:
				Label = ctk.CTkLabel(my_S_Frame, text=i2, text_color="light green", fg_color="black", font=my_font)
				Label.pack(padx=5, pady=1, side= TOP, anchor="w")
		else:
			infoLabel = ctk.CTkLabel(my_S_Frame, text="No keys were imported from Secure archive to the local keychain.", text_color="light green", fg_color="black", font=my_font)
			infoLabel.pack(padx=5, pady=1, side= TOP, anchor="w")
	...
	def exportGPG(self):
		...
		Label1.place(relx=0.05, rely=0.14, anchor="center")
		...
		Label2.place(relx=0.05, rely=0.24, anchor="center")
		...
		Label3.place(relx=0.05, rely=0.32, anchor="center")
	...
	def select_what_to_sign(self):
		...
		Button = ctk.CTkButton(my_Frame, text="Encrypt a document", text_color="white", fg_color=GPG_button_color, border_width=2, border_color="white", width=370, height=35, font=my_font, command=self.encrypt_file)
		Button.place(relx=0.3, rely=0.1, anchor="center")
		
		Button4 = ctk.CTkButton(my_Frame, text="Encrypt and sign a document", text_color="white", fg_color=GPG_button_color, border_width=2, border_color="white", width=370, height=35, font=my_font, command=self.encrypt_and_sign_file)
	...
	def BTC_timestamp(self):
		...
		tk.messagebox.showinfo("Information", "Insert USB-device and then click \"OK\".")
		time.sleep(2)
		tk.messagebox.showinfo('Information', 'Select the file to make BTC Timestamp address for.')
		time.sleep(2)
		filepathSourcefile = filedialog.askopenfilename(initialdir='/media/user1')
	...
	def encrypt_and_sign_file(self):
		...
		Label1 = ctk.CTkLabel(my_Frame, text="Encrypt and sign a file using one of the public keys on the local keychain.", text_color="white", fg_color="black", font=my_font)
		Label1.place(relx=0.05, rely=0.1, anchor="w")
		my_font = ctk.CTkFont(family="Arial", size=18, slant="roman", underline=False, overstrike=False)
		...
		if publickeysavailable == True:
			...
			Button2.place(relx=0.82, rely=0.54, anchor="e")
			Button2 = ctk.CTkButton(my_Frame, text="Select file on Offline device", text_color="white", fg_color=GPG_button_color, font=my_font, border_width=2, border_color="white", command=do_encrypt_and_sign_file_on_OD)
			Button2.place(relx=0.82, rely=0.6, anchor="e")
		else:
			Button3 = ctk.CTkButton(my_Frame, text="There are no public keys on the keychain. Create one now?", font=my_font, text_color="white", fg_color="green", border_width=2, border_color="white", command=self.importGPG_Key)
			Button3.place(relx=0.5, rely=0.55, anchor="center")
	...
	def encrypt_file(self):
		...
		if publickeysavailable == True:
			...
			Button2.place(relx=0.79, rely=0.48, anchor="e")
			...
			Button3.place(relx=0.79, rely=0.54, anchor="e")
		else:
	...
	def new_secureUSB(self):
		...
		Button.place(relx=0.47, rely=0.53, anchor="w")
		my_font = ctk.CTkFont(family="Arial", size=18, slant="roman", underline=False, overstrike=False)
								...
								if os.path.getsize(path_to_archive) > 1000:
									if os.path.isfile(path_to_privatekeyfile):
										if os.path.getsize(path_to_privatekeyfile) > 1000:
											...
											messagebox.showinfo("Information", statusText)
	...
	def restore_SecUSB(self):
		...
		if path_to_USB_secure == 'Secure USB folder is available':
			...
			Label2inf = ctk.CTkLabel(my_Frame, text="After restoration you need to manually restore the keys from archive to the local keychain.", text_color="white", fg_color="black", font=my_font)
			Label2inf.place(relx=0.5, rely=0.32, anchor="center")
	...
	def restoreFromencrypted_SecUSB(self):
		...
		def do_restoreFromencrypted_SecUSB():
			...
			messagebox.showinfo("Information", "Now select the encrypted backup archive (could end with \".securefolder.tar.gz.gpg\" or other).")
			time.sleep(2)
	...
	def check_SecUSB(self, key):
		...
		my_text = ctk.CTkTextbox(my_Frame, width=1200, height=650, corner_radius=1, border_width=1, border_color="purple", border_spacing=10, fg_color="black", text_color="white", font=("Helvetica", 20), wrap="word", activate_scrollbars = True, scrollbar_button_color="blue", scrollbar_button_hover_color="red")
		...
		if path_to_USB_secure == 'Secure USB folder is available':
			...
			Button.place(relx=0.05, rely=0.05, anchor="w")
			...
			Button3 = ctk.CTkButton(my_Frame, text="Add directory", text_color="white", fg_color=SecUSB_button_color, border_width=2, border_color="white", font=my_font, command=self.add_directory_SecUSB)
			Button3.place(relx=0.35, rely=0.05, anchor="w")
			Button4 = ctk.CTkButton(my_Frame, text="Remove directory", text_color="white", fg_color=SecUSB_button_color, border_width=2, border_color="white", font=my_font, command=self.remove_directory)
			Button4.place(relx=0.5, rely=0.05, anchor="w")
			...
			Button5.place(relx=0.85, rely=0.05, anchor="center")
			...
			Button6.place(relx=0.85, rely=0.1, anchor="center")
			my_font = ctk.CTkFont(family="Arial", size=22, weight="bold", slant="roman", underline=True, overstrike=False)
			filterLabel = ctk.CTkLabel(my_Frame, text="Filter:", text_color="white", font=my_font, fg_color="black")
			filterLabel.place(relx=0.85, rely=0.2, anchor="e")
			my_font = ctk.CTkFont(family="Arial", size=16, weight="bold", slant="roman", underline=False, overstrike=False)
			...
			editButton.place(relx=0.87, rely=0.2, anchor="w")
			...
			if os.path.isfile(filter_path):
				# Look in filter CVS-file for different filters (name of filter and the full path to dierctory)
				my_font = ctk.CTkFont(family="Arial", size=16, slant="roman", underline=False, overstrike=False)
				Counting = 0
				try:
			...
			if key == 'none':
				...
				my_font = ctk.CTkFont(family="Arial", size=22, slant="roman", weight='bold' , underline=False, overstrike=False)
				sizeLabel = ctk.CTkLabel(my_Frame, text='Secure archive size: ' + answer + ' MB', text_color="white", font=my_font, fg_color="black")
				sizeLabel.place(relx=0.5, rely=0.12, anchor="center")		
			else:
				# List the SELECTED directory on the Secure USB
	...
	def filter_SecUSB(self):
		...
		def add_filter():
			messagebox.showinfo("Information", "Select the folder to add filter for.")
			...
			if USER_INP == "" or USER_INP == " ":
				filtername_list = folder_path.split("/")
				USER_INP = str(filtername_list[-1])
	...
	def copyfile_USB_to_USB(self):
		global filepathdestinationfolder
		messagebox.showinfo("Information", "Insert both the USB-devices.") 
		time.sleep(2)
		messagebox.showinfo("Information", "Select a file to copy FROM the first USB.")
		time.sleep(2)
		source = filedialog.askopenfilename(initialdir='/media/user1')
		messagebox.showinfo("Information", "Select the TO destination by double clicking on it.")
