...
class Menu(ctk.CTkFrame):	
	...
	def inheritance_pre(self):
		...
		Label1 = ctk.CTkLabel(my_Frame, text="Make your inheritance package", text_color="white", fg_color="black", font=my_font)
		Label1.place(relx=0.5, rely=0.14, anchor="center")
		...
		my_font = ctk.CTkFont(family="Arial", size=20, slant="roman", underline=False, overstrike=False)
		Label33 = ctk.CTkLabel(my_Frame, text="(This gives the beneficiary the possibility to check WHEN you signed it using the Bitcoin blockchain)", text_color="white", fg_color="black", font=my_font)
		Label33.place(relx=0.5, rely=0.38, anchor="center")
		Label3 = ctk.CTkLabel(my_Frame, text="There is an option to send a bitcoin (timestamp) transaction to an address calculated by the signature and archive files.", text_color="white", fg_color="black", font=my_font)
		...
		theButton = ctk.CTkButton(my_Frame, text="Start!", text_color="white", fg_color=SecUSB_button_color, border_width=2, border_color="white", font=my_font, command=self.inheritance)
	...
	def inheritance(self):
		...
		def do_newInheritance():
			...
			if last_will_checkbox.get() == "on":
				...
				ODWalletspath = filepathdestinationfolder + '/secure/temparchiveforinheritance0/bitconwallets.csv'
				SingleWalletspath = filepathdestinationfolder + '/secure/temparchiveforinheritance0/paperwallets/paperwallets.csv'
				...
				if checkboxWallets.get() == 'off':
					c = open(Walletspath, 'w')
					c.write(str("Wallets\n___________________________________\n\nName: \n\n   Seed words/key: \n\n   Comment: \n\n___________________________________\n\nName: \n\n   Seed words/key: \n\n   Comment: \n"))
					c.close()
					
				if checkboxODWallets.get() == 'off':
					with open(ODWalletspath, 'w', newline='') as file:
						writer = csv.writer(file)
						
				if checkboxSingleWallets.get() == 'off':
					with open(SingleWalletspath, 'w', newline='') as file:
						writer = csv.writer(file)					
				
				if checkboxBolt.get() == 'off':
				...
				# Make a Bitcoin address based on the sum256 hashed value of archive and store in single Bitcoin address wallet
		...
		if privatekeysavailable == True:
			...
			tButton.place(relx=0.61, rely=0.33, anchor="center")
			...
			doButton.place(relx=0.65, rely=0.94, anchor="w")
	...
	def create_DigitalIDbox(self):
		...
		if FirstID != 'empty':
			...
			AddkeyButton = ctk.CTkButton(right_Frame, text="Create new encryption key for ID", text_color="white", fg_color=GPG_button_color, border_width=2, border_color="white", width=170, height=32, font=my_font, command=self.newGPGFull_Key)
