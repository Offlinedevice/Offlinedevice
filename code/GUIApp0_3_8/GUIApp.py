...
class Menu(ctk.CTkFrame):	
	...
	def create_BitcoinWalletmeny(self):
		...
		if path_to_USB_secure == 'Secure USB folder is available':
			...
			infoLabel12 = ctk.CTkLabel(right_Frame, text="  - Click on the new wallet and select \"Show Xpub\". Scan QR or save to file and import to your online device **.", text_color="white", font=my_font, fg_color="black")
			...
			proLabel2 = ctk.CTkLabel(right_Frame, text="** A \"online device\" is a seperate device that can create and broadcast transactions.", text_color="white", font=my_font, fg_color="black")
			proLabel2.place(relx=0.03, rely=0.9, anchor="w")
			proLabel3 = ctk.CTkLabel(right_Frame, text="   Having your own node and connecting through a local network protects your privacy!", text_color="white", font=my_font, fg_color="black")
				...
				with open(completeName, 'r') as file:
					...
					for lines in csvfile:
						if lines[0] == walletname:	
							...
							if lines[5] != "False":
								my_font = ctk.CTkFont(family="Arial", size=20, weight="bold", slant="roman", underline=True, overstrike=False)
								passwordLabel = ctk.CTkLabel(right_Frame, text="BIP38 Passphrase:", text_color="white", font=my_font, fg_color="black")
							...
							if lines[4] == "12":
								...
								seedQRButton.place(relx=0.5, rely=0.76, anchor="center")
	...
	def signBitcoinMessage(self, walletname):
		...
		tk.messagebox.showinfo('Information', 'Connect the USB-device with the message and then click \"OK\".')
		time.sleep(2)
		messagebox.showinfo("Information", "Select the message to sign with your wallet private key.")
		time.sleep(2)
				...
				with open(completeName, 'r') as file:
					...
					for lines in csvfile:
						if lines[0] == walletname:
							mnemo = Mnemonic("english")
							if lines[5] == "False":
								theHDKey = HDKey.from_passphrase(lines[3])
							else:
								theHDKey = HDKey.from_passphrase(lines[3], password=lines[6])
							
							key = CBitcoinSecret(theHDKey.wif_key())
							
							filename_list = pathtomessage.split("/")
							filenameMessage = str(filename_list[-1])
							
							message = BitcoinMessage(msg)
							signatureForMessage = SignMessage(key, message)
							
							tk.messagebox.showinfo('Information', 'Select the directory for the signed message file.')
							time.sleep(2)
							outputdir = filedialog.askdirectory(initialdir='/media/user1')
							
							with open(outputdir + '/' + filenameMessage + '.sig', "wb") as binary_file:
								binary_file.write(signatureForMessage)
									
							messagebox.showinfo('Information', 'Signature file \"' + filenameMessage + '.sig\"' + ' has been saved.')
			except FileNotFoundError:
				messagebox.showinfo("Information", "There are no Bitcoin wallets here yet.")
		self.showBitcoinwallet(walletname)
